  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success || response.ok) {
        setUser({
          id: result.user ? String(result.user.id) : `user_${Math.random().toString().slice(2,8)}`,
          email: result.user ? result.user.email : email,
          fullName: result.user ? result.user.fullName : name,
        });
      ...
      <FieldGroup>
        ...
        <Field>
          ...
          <FieldDescription className="px-6 text-center">
            Already have an account? <a href={`/login${searchParams.toString() ? `?${searchParams.toString()}` : ''}`} className="underline underline-offset-4 text-orange-600 font-medium">Sign in</a>
          </FieldDescription>
        </Field>
      </FieldGroup>
    </form>
