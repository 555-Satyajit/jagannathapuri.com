...
export default async function Footer() {
  ...
  try {
    const configs = await prisma.siteConfig.findMany({
      where: {
        key: {
          in: ['contact_phone', 'contact_email', 'contact_address', 'facebook_link', 'instagram_link', 'youtube_link', 'header']
        }
      }
    });
  ...
  const phone = configMap['contact_phone'] || "+91 98765 43210";
  const email = configMap['contact_email'] || "support@jaysubhdra.com";
  const address = configMap['contact_address'] || "Grand Road, Puri, Odisha 752001, India";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
            ...
            <Link href="/" className="mb-6 inline-block">
              {logoUrl ? (
                ...
                <span className="text-2xl md:text-3xl font-serif font-bold text-orange-600 tracking-tight">
                  Jay <span className="text-zinc-900">Subhdra</span>
                </span>
              )}
            </Link>
            <p className="text-zinc-600 leading-relaxed mb-8">
              Bringing the authentic spiritual heritage and sacred treasures from the divine city of Puri directly to your doorstep.
            </p>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <a href={configMap['facebook_link'] || "#"} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-600 hover:bg-orange-600 hover:text-white transition-all shadow-sm">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                </svg>
              </a>
              <a href={configMap['instagram_link'] || "#"} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-600 hover:bg-orange-600 hover:text-white transition-all shadow-sm">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <a href={configMap['youtube_link'] || "#"} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-600 hover:bg-orange-600 hover:text-white transition-all shadow-sm">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div className="border-t border-zinc-200 bg-zinc-50">
        <div className="container max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-zinc-500 text-sm text-center md:text-left">
            &copy; {new Date().getFullYear()} Jay Subhdra. All rights reserved.
          </p>
