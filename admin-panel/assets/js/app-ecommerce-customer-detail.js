"use strict"; !function () {
    var t = document.querySelector(".delete-customer"), t = (t && (t.onclick = function () {
        Swal.fire({ title: "Are you sure?", text: "You won't be able to revert customer!", icon: "warning", showCancelButton: !0, confirmButtonText: "Yes, Delete customer!", customClass: { confirmButton: "btn btn-primary me-2", cancelButton: "btn btn-label-secondary" }, buttonsStyling: !1 }).then(function (t) {
            if (t.value) {
                ...
                fetch('/admin/ecommerce/customers/delete/' + customerId, { method: 'DELETE' })
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({ icon: "success", title: "Deleted!", text: "Customer has been removed.", customClass: { confirmButton: "btn btn-success" } })
                                .then(() => { window.location.href = '/admin/ecommerce/customers'; });
                        } else {
                            Swal.fire({ icon: "error", title: "Error!", text: data.error || "Failed to delete customer.", customClass: { confirmButton: "btn btn-success" } });
                        }
                    }).catch(err => {
                        Swal.fire({ icon: "error", title: "Error!", text: "An error occurred while deleting.", customClass: { confirmButton: "btn btn-success" } });
                    });
            } else t.dismiss === Swal.DismissReason.cancel && Swal.fire({ title: "Cancelled", text: "Cancelled Delete :)", icon: "error", customClass: { confirmButton: "btn btn-success" } })
        })
